...
class Review extends StatelessWidget {
  ...
  int kills(List<DotaMatch> matches) {
    return matches.fold(
        0, (prev, element) => prev + element.players[0].numKills);
  }

  int deaths(List<DotaMatch> matches) {
    return matches.fold(
        0, (prev, element) => prev + element.players[0].numDeaths);
  }

  int assists(List<DotaMatch> matches) {
    return matches.fold(
        0, (prev, element) => prev + element.players[0].numAssists);
  }
      ...
      children: <Widget>[
        Column(
          children: <Widget>[
            ...
            KDA(kills(info.matches), deaths(info.matches),
                assists(info.matches)),
          ],
        ),
