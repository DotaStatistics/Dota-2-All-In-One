...
List<prefix0.Player> getRadiantPlayers(MatchDetails matchDetails){
  return matchDetails.players.where((player) => player.isRadiant == true).toList();
}

List<prefix0.Player> getDirePlayers(MatchDetails matchDetails){
  return matchDetails.players.where((player) => player.isRadiant == false).toList();
}
            ...
            body: Center(
              ...
              child: Column(
                children: <Widget> [
                  ...
                  TeamDetails(getRadiantPlayers(snapshot.data),widget.rolesAndGameModes["roles"]),
                  Text("Team Dire",
                  style: FontStyles.whiteTextHeader()),
                  TeamDetails(getDirePlayers(snapshot.data),widget.rolesAndGameModes["roles"])
                ]
              )
            )
          ));
